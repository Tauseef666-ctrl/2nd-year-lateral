...
import { ArrowRight, ExternalLink, ListVideo } from 'lucide-react';
...
const KIND_REQUESTED: ResourceKind = 'playlist';
...
export default function Playlists() {
  ...
  const rows = useMemo(
    () =>
      FLAT_TOPICS.map((t) => ({
        topic: t,
        lists: t.resources.filter((r) => r.kind === KIND_REQUESTED),
      })).filter((r) => r.lists.length > 0),
    [],
  );
  ...
  return (
    <section className="space-y-6">
      <PageHeader
        ...
        subtitle="Multi-video courses and full-unit playlists for every topic. Follow these for structured, in-depth coverage."
      >
        <div className="inline-flex items-center gap-1.5 self-end rounded-xl bg-accent/10 px-3 py-1.5 text-xs font-bold text-accent dark:text-accent-300">
          <ListVideo className="h-3.5 w-3.5" />
          {rows.length} topics with playlists
        </div>
      </PageHeader>
      <SourceNote />
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          body="Try another subject filter, or open the full topic page for deep and one-shot resources."
        />
      ) : (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map(({ topic, lists }) => {
            const s = getSubject(topic.subjectId);
            const firstQuery = lists[0].query;
            return (
              <Card key={topic.id} className="flex flex-col p-5 transition hover:shadow-cardHover">
                ...
                <p className="mt-1 text-xs font-medium text-ink-400">
                  {s?.shortName}
                  {lists[0]?.language ? ` · ${lists[0].language}` : ''}
                  {lists[0]?.duration ? ` · ${lists[0].duration}` : ''}
                </p>
